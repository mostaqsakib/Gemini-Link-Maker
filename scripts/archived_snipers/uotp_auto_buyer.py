...
async def buy_batch(session, count):
    print(f"\n{C.CYN}{C.B}▸ Attempting to buy {count} numbers...{C.RST}")
    results = []
    remaining = count
    for server in JIO_SERVERS:
        if remaining <= 0: break
        
        print(f"  {C.DIM}Trying Server {server['id']} (₹{server['price']})...{C.RST}")
...
async def cancel_task(session, num_info):
    ...
    print(f"  {C.DIM}[{phone}] Scheduled for cancellation. Waiting 120 seconds...{C.RST}")
    ...
    while True:
        ...
        if status == "EARLY_CANCEL_DENIED":
            print(f"  {C.YEL}[{phone}] Cancel denied (too early). Retrying in 10s...{C.RST}")
...
async def poll_task(session, num_info):
    ...
    while True:
        ...
        if status.startswith("STATUS_OK:"):
            ...
            print(f"  {C.YEL}[{phone}] Number was cancelled by the server.{C.RST}")
            break
        elif "ERROR" in status:
            print(f"  {C.RED}[{phone}] Polling error: {status}{C.RST}")
            await asyncio.sleep(10)
        else:
            # STATUS_WAIT_CODE
...
async def main_flow(count):
    async with aiohttp.ClientSession() as session:
        ...
        if bal is None:
            ...
            return
        print(f"{C.GRN}💰 Current Balance: ₹{bal}{C.RST}")
        
        # 1. Buy numbers
        purchased = await buy_batch(session, count)
        if not purchased:
            print(f"{C.RED}❌ Could not purchase any numbers. All servers out of stock.{C.RST}")
            return
        ...
        # We process them sequentially or parallelly. Let's do parallel for speed.
        ...
        for p, carrier_name in zip(purchased, carriers):
            ...
            # Jio networks might say "Reliance Jio", "Reliance Telecom", etc.
...
def main():
    parser = argparse.ArgumentParser(description="JIO Ultimate Auto-Buyer with Live MNP")

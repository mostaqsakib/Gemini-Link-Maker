...
def adb_type_otp(otp_code):
    print(f"  {C.CYN}🤖 ADB: Typing OTP {otp_code} into MyJio App...{C.RST}")
...
async def buy_batch(session, count):
    ...
    if errors and not results:
        # If everything failed, print the first error to debug
        print(f"  {C.DIM}Tiger SMS API Response: {errors[0]['raw']}{C.RST}")
...
async def poll_task(session, num_info):
    aid = num_info["activation_id"]
    phone = num_info["phone_number"]
    print(f"\n  {C.GRN}{C.B}★ [{phone}] TRUE JIO SECURED!{C.RST}")
    
    # Trigger ADB Hardware Automation
...
async def main_flow(target_jio_count):
    # Warn if no ADB device is connected
    try:
        output = subprocess.check_output("adb devices", shell=True, text=True)
        if output.strip().split('\n')[-1] == "List of devices attached":
            print(f"{C.RED}{C.B}WARNING: No ADB device or emulator detected! ADB commands will silently fail.{C.RST}")
    except:
        pass
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if bal is None:
            print(f"{C.RED}Failed to fetch Tiger SMS balance. Check API key.{C.RST}")
            return
        print(f"{C.GRN}💰 Tiger SMS Auto-Sniper Started! Target: {target_jio_count} true Jio accounts. Current Balance: ${bal}{C.RST}")
        ...
        try:
            # We don't want to buy too many at once to avoid rate limits
            batch_size = target_jio_count * 3
            if batch_size > 5: batch_size = 5
            
            while true_jio_secured < target_jio_count:
                sys.stdout.write(f"\r{C.DIM}▸ Sniping Tiger SMS... Looking for stock...{C.RST}")
                sys.stdout.flush()
                ...
                if true_jio_secured < target_jio_count:
                    # Tiger API asks to reduce concurrent requests, so we sleep for 5 seconds between batches
                    await asyncio.sleep(5)
...
def main():
    parser = argparse.ArgumentParser(description="Tiger SMS Jio Infinite Sniper + ADB Automator")
